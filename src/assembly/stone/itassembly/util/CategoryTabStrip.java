...
public class CategoryTabStrip extends HorizontalScrollView {
	...
	public OnDoubleTouchListenter onDoubleTouchListenter;
	...
	private void addTab(final int position, String title) {
		...
		category_text.setTextColor(getResources().getColor(R.color.category_tab_text_dark));
				...
				if (MotionEvent.ACTION_DOWN == event.getAction()) {
					...
					if (count == 1) {
						...
						if (secClick - firClick < 1000) {
							// 双击事件s
							onDoubleTouchListenter.onDoubleTouch(pager.getCurrentItem());
						} else {
							pager.setCurrentItem(position);
						}
						count = 0;
	...
	private void calculateIndicatorRect(Rect rect) {
		...
		rect.set(((int) left) + getPaddingLeft(), getPaddingTop() + currentTab.getTop() + category_text.getTop(),
				((int) width) + getPaddingLeft(), currentTab.getTop() + getPaddingTop() + category_text.getTop()
		...
		while (i < tabsContainer.getChildCount()) {
			if (i < currentPosition - 1 || i > currentPosition + 1) {
				...
				if (category_text != null) {
					...
					textDrawable.setTextColor(getResources().getColor(R.color.category_tab_highlight_bg));
					...
					if (i == 4) {
						textDrawable.setBounds(left + 1, top + 1, textDrawable.getIntrinsicWidth() + left,
								textDrawable.getIntrinsicHeight() + top);
					} else {
						textDrawable.setBounds(left, top + 1, textDrawable.getIntrinsicWidth() + left,
								textDrawable.getIntrinsicHeight() + top);
					}
	...
	public OnDoubleTouchListenter getOnDoubleTouchListenter() {
		return onDoubleTouchListenter;
	}

	public void setOnDoubleTouchListenter(OnDoubleTouchListenter onDoubleTouchListenter) {
		this.onDoubleTouchListenter = onDoubleTouchListenter;
	}
